...
import { sendPinCode } from "src/app/store-management/actions/accounts/accounts-actions";
import Card from "../card/card-component";
import cardHeader from "../card/card-header";
import cardFooter from "../card/card-footer";
import logoHeaderCard from '../../../assets/image-octopusfx/Logo Octopus-col_Plan de travail 1.png'
import imgVerifyEmail from '../../../assets/image-octopusfx/confirm-email.svg'
import imgVerifyNumber from '../../../assets/image-octopusfx/confirm-phone.svg'
import { useAppDispatch, useAppSelector } from "src/app/core/hooks/core-hooks";
import { isEmail } from "src/app/core/text/regex";
import { VerificationTypeEnum } from "src/app/core/enums/enums-core";
...
function ChangeContactMediaComponent({ description="", name, closeDialog }: Props) {
  ...
  const acceptChangeContactDone = async () => {
    ...
    if (
      values.contact === "" ||
      (name === "email" && !isEmail(values.contact))
    ) {
      let contactError = "";
      if (name === "email" && !isEmail(values.contact))
